...
export default function WeeklyExpenses() {
    ...
    async function fetchWeeklyExpenses() {
        try {
            setLoading(true);
            const data = await apiService.getTransactions({ category: 'weekly' });
            setItems(data);
        } catch (err) {
    ...
    async function handleAdd(e) {
        ...
        try {
            ...
            console.error("Error creating weekly expense:", err);
            setError("Failed to add weekly expense.");
        }
    }
    ...
    async function handleRemove(id) {
        try {
            ...
            console.error("Error deleting weekly expense:", err);
            setError("Failed to delete weekly expense.");
        }
    }
        ...
        <>
            <div className="Main">
                    ...
                    <div className="current">
                        <div className="setion">
                            <p className="anoutname">
                                Total Weekly: INR {loading ? "..." : totalAmount.toFixed(2)}
                            </p> 
                            <br />
                            <p className="anoutname">
                ...
                <div className="listofmonltyexpenses">
                    <div>
                        <h1>List of Weekly expenses</h1>
                    </div>
                    {loading ? (
                        <p style={{ color: "#888", textAlign: "center", padding: "10px" }}>Loading...</p>
                    ) : items.length === 0 ? (
                        <p style={{ color: "#888", textAlign: "center", padding: "10px" }}>No weekly expenses added yet.</p>
                    ) : (
